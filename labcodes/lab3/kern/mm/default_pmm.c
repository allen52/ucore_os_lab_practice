...
default_init_memmap(struct Page *base, size_t n) {
    ...
    struct Page *p = base;
    for (; p != base + n; p ++) {
        assert(PageReserved(p));//检查该内存是否被reserved
        p->flags = p->property = 0;//被reserved 且某连续内存空闲块的大小 = 0
        set_page_ref(p, 0);//引用数为0
    }
...
default_alloc_pages(size_t n) {
    ...
    while ((le = list_next(le)) != &free_list) {//从free_list后面开始找，直到循环回到初始指针位置
        struct Page *p = le2page(le, page_link);//由指针定位到当前page
        if (p->property >= n) {
            page = p;//如果这个page的连续内存空间大于n 跳出循环
            break;
        }
    }
    if (page != NULL) {
    ...
    return page;//返回该page，由该page可以用page2pa（page，page->property）快速获得连续物理内存
}
...
default_free_pages(struct Page *base, size_t n) {
    assert(n > 0);
    struct Page *p = base;
    for (; p != base + n; p ++) {//base后n个页表
        assert(!PageReserved(p) && !PageProperty(p));//检测该内存是否被占用，且连续内存块大于0
        p->flags = 0;//flags置0
        set_page_ref(p, 0);//引用置0
    }
    base->property = n;//base后n个内存块为空闲
    SetPageProperty(base);//声明空闲
    list_entry_t *le = list_next(&free_list);
    //在free_list循环链表内查找，如果发现base+base->property 这种连续内存块
    //第一种情况：base+base->property等于p
    //第二种情况：p+p->property等于base
    while (le != &free_list) {
        p = le2page(le, page_link);
        le = list_next(le);//循环 找符合条件的le
        // TODO: optimize
        if (base + base->property == p) {//找到连续内存块
            base->property += p->property;//base后面直接加p的数量
            ClearPageProperty(p);//把p清除
            list_del(&(p->page_link));//空闲链表中删除p
        }
        else if (p + p->property == base) {
            p->property += base->property;//p后面直接加base
            ClearPageProperty(base);//把base清除
            base = p;//把p赋给base
